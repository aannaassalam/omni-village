...
const DemographicDisease = ({navigation, route}) => {
  ...
  const scheme = yup.object().shape({
    chronic_disease: yup
      .array()
      .required(t('chronic disease is required'))
      .min(1, t('Atleast one chronic disease is required')),
    ...
    feelings_with_others: yup
      .string()
      .required(t('feelings with others is required')),
    support_you_have: yup
      .array()
      .required(t('support you have is required'))
      .min(1, t('atleast one support is required')),
    recover_from_stress: yup
      .string()
      .required(t('recover from stress is required')),
    share_feelings_of_others: yup
      .string()
      .required(t('share feelings of others is required')),
  });
  const {
    handleChange,
    handleSubmit,
  ...
  useEffect(() => {
    resetForm({
      values: {
        chronic_disease:
          data?.general_data?.chronic_disease?.map(i => {
            return i?._id;
          }) || [],
        ...
        currently_feeling:
          data?.mental_and_emotional_wellbeing?.currently_feeling?._id || '',
        feelings_with_others:
          data?.mental_and_emotional_wellbeing?.feelings_with_others?._id || '',
        support_you_have:
          data?.mental_and_emotional_wellbeing?.support_you_have.map(i => {
            return i?._id;
          }) || [],
        recover_from_stress:
          data?.mental_and_emotional_wellbeing?.recover_from_stress?._id || '',
        share_feelings_of_others:
          data?.mental_and_emotional_wellbeing?.share_feelings_of_others?._id ||
          '',
      },
    });
  }, [data]);
    ...
    <View style={styles.container}>
      ...
      <KeyboardAwareScrollView
        ...
        <MultiselectDropdown
          ...
          data={dropdownData?.['chronic_diseases'].map(item => {
        ...
        {dropdownData?.['chronic_diseases'].find(item =>
          values?.chronic_disease.includes(item?._id),
        )?.name === 'Others(if any)' && (
          <Input
        ...
        <Customdropdown
          data={dropdownData?.['motor_disability'].map(item => {
        ...
        {dropdownData?.['motor_disability'].find(
          item => item?._id === values?.motor_disablity,
        ...
        <View style={styles.subArea}>
          <Text
            ...
            {t('Mental Health & Emotional Well-being')}
          </Text>
          <Divider
            bold={true}
        ...
        {mental ? (
          <View style={styles.innerInputView}>
            ...
            <View style={{width: '100%'}}>
              <Customdropdown
                data={dropdownData?.['overall_wellbeing'].map(item => {
                  return {id: item?._id, label: item?.name, value: item?._id};
                })}
              ...
              {touched?.currently_feeling && errors?.currently_feeling && (
                <Text style={Styles.error2}>
                  {String(errors?.currently_feeling)}
                </Text>
              )}
              <Customdropdown
                data={dropdownData?.['communication_of_feelings'].map(item => {
                  return {id: item?._id, label: item?.name, value: item?._id};
                })}
              ...
              />
              {touched?.feelings_with_others &&
                errors?.feelings_with_others && (
                  <Text style={Styles.error2}>
                    {String(errors?.feelings_with_others)}
                  </Text>
                )}
              <MultiselectDropdown
                containerStyle={{
                ...
                data={dropdownData?.['support_system'].map(item => {
                  return {name: item?.name, key: item?._id};
                })}
                setSelectedd={item =>
              ...
              {touched?.support_you_have && errors?.support_you_have && (
                <Text style={Styles.error2}>
                  {String(errors?.support_you_have)}
                </Text>
              )}
              <Customdropdown
                data={dropdownData?.['stress_and_resilience'].map(item => {
                  return {id: item?._id, label: item?.name, value: item?._id};
                })}
              ...
              {touched?.recover_from_stress && errors?.recover_from_stress && (
                <Text style={Styles.error2}>
                  {String(errors?.recover_from_stress)}
                </Text>
              )}
              <Customdropdown
                data={dropdownData?.['empathy'].map(item => {
                  return {id: item?._id, label: item?.name, value: item?._id};
                })}
                value={values.share_feelings_of_others}
                label={t(
                  'How well do you understand & share feelings of others?',
                )}
                onChange={value => {
                  setValues({
                    ...values,
              ...
              />
              {touched?.share_feelings_of_others &&
                errors?.share_feelings_of_others && (
                  <Text style={Styles.error2}>
                    {String(errors?.share_feelings_of_others)}
                  </Text>
                )}
            </View>
          </View>
