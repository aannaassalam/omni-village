...
const DemographicUnfulfilled = ({navigation, route}) => {
  ...
  useEffect(() => {
    resetForm({
      values: {
        basic_necessities:
          data?.unfulfilled_needs?.basic_necessities.map(i => {
            return i?._id;
          }) || [],
        educational_needs:
          data?.unfulfilled_needs?.educational_needs.map(i => {
            return i?._id;
          }) || [],
        economic_needs:
          data?.unfulfilled_needs?.economic_needs.map(i => {
            return i?._id;
          }) || [],
        healthcare_needs:
          data?.unfulfilled_needs?.healthcare_needs.map(i => {
            return i?._id;
          }) || [],
        infrastructure_needs:
          data?.unfulfilled_needs?.infrastructure_needs.map(i => {
            return i?._id;
          }) || [],
        social_governance_needs:
          data?.unfulfilled_needs?.social_governance_needs.map(i => {
            return i?._id;
          }) || [],
        environmental_needs:
          data?.unfulfilled_needs?.environmental_needs.map(i => {
            return i?._id;
          }) || [],
          ...
          <View style={styles.innerInputView}>
            ...
            <View style={{width: '100%'}}>
              <MultiselectDropdown
                ...
                data={dropdownData?.['basic_necessities'].map(item => {
              ...
              {touched?.economic && errors?.basic_necessities && (
                <Text style={Styles.error2}>
                  {String(errors?.basic_necessities)}
                </Text>
              )}
              <MultiselectDropdown
                containerStyle={{
                ...
                data={dropdownData?.['educational_needs'].map(item => {
                  return {name: item?.name, key: item?._id};
                })}
                setSelectedd={item =>
              ...
              {touched?.educational_needs && errors?.educational_needs && (
                <Text style={Styles.error2}>
                  {String(errors?.educational_needs)}
                </Text>
              )}
              <MultiselectDropdown
                containerStyle={{
                ...
                data={dropdownData?.['economic_needs'].map(item => {
                  return {name: item?.name, key: item?._id};
                })}
                setSelectedd={item =>
              ...
              <MultiselectDropdown
                ...
                data={dropdownData?.['healthcare_needs'].map(item => {
                  return {name: item?.name, key: item?._id};
                })}
                setSelectedd={item =>
              ...
              {touched?.healthcare_needs && errors?.healthcare_needs && (
                <Text style={Styles.error2}>
                  {String(errors?.healthcare_needs)}
                </Text>
              )}
              <MultiselectDropdown
                containerStyle={{
                ...
                data={dropdownData?.['infrastructure_needs'].map(item => {
                  return {name: item?.name, key: item?._id};
                })}
                setSelectedd={item =>
              ...
              />
              {touched?.infrastructure_needs &&
                errors?.infrastructure_needs && (
                  <Text style={Styles.error2}>
                    {String(errors?.infrastructure_needs)}
                  </Text>
                )}
              <MultiselectDropdown
                containerStyle={{
                ...
                data={dropdownData?.['social_and_governance_needs'].map(
                  item => {
                    return {name: item?.name, key: item?._id};
                  },
                )}
              ...
              />
              {touched?.social_governance_needs &&
                errors?.social_governance_needs && (
                  <Text style={Styles.error2}>
                    {String(errors?.social_governance_needs)}
                  </Text>
                )}
              <MultiselectDropdown
                containerStyle={{
                ...
                data={dropdownData?.['environmental_needs'].map(item => {
                  return {name: item?.name, key: item?._id};
                })}
                setSelectedd={item =>
              ...
              {touched?.environmental_needs && errors?.environmental_needs && (
                <Text style={Styles.error2}>
                  {String(errors?.environmental_needs)}
                </Text>
              )}
              <Input
