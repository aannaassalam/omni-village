...
export default function RegisterDetailsFieldOfficer({ navigation, route }) {
    // const countries = ['Egypt', 'Canada', 'Australia', 'Ireland'];
    const [fileResponse, setFileResponse] = useState([]);
    const [file_err, setFile_err] = useState('');
    const [fileResponseOfficer, setFileResponseOfficer] = useState([]);
    const [file_err_officer, setFile_err_officer] = useState('');
    ...
    const handleDocumentSelection = useCallback(async (type) => {
        try {
            ...
            if(type=="officer"){
                setFile_err_officer('');
                setFileResponseOfficer(response);
            }else{
                setFile_err('');
    ...
    const FormSubmit = data => {
        if (fileResponse.length === 0 && !isEdit) {
            console.log('in');
            setFile_err('Please select a address document!');
            return;
        }

        if (fileResponseOfficer.length === 0 && !isEdit) {
            console.log('inofficer');
            setFile_err_officer('Please select a officer document!');
            return;
        }
        ...
        const _file_officer = fileResponseOfficer[0]||{}
        ...
        formData.append('officer_proof', {
            uri: _file_officer?.uri || '',
            type: _file_officer?.type || '',
            filename: _file_officer?.name || '',
            name: 'officer_proof',
        });
          ...
          /> */}
                ...
                {!isEdit && (
                    <>
                        ...
                        <Box style={[styles.file_box,{marginTop: '3%'}]}>
                            <Box style={styles.file_box_lft}>
                                <Image
                                    style={styles.tinyLogo}
                                    source={require('../../../assets/file_img.png')}
                            ...
                            <Box style={styles.file_box_rgt}>
                                ...
                                <TouchableOpacity
                                    style={styles.btn}
                                    onPress={()=>handleDocumentSelection('officer')}>
                                    <Text style={styles.cmn_btn_text}>{t('browse')}</Text>
                                </TouchableOpacity>
                            </Box>
                        </Box>
                        {file_err_officer.length > 0 && (
                            <Text style={styles.error}>{file_err_officer}</Text>
                        )}
                        {fileResponseOfficer.map((file, index) => (
                            <Box style={styles.file_box2} key={file.name}>
                                <Box style={styles.file_box_lft}>
                                    <Image
                                        style={styles.tinyLogo}
                                    ...
                                    <Pressable onPress={() => setFileResponseOfficer([])}>
                                        <Text
                                            style={{
                                                ...styles.error,
                                                marginTop: 0,
