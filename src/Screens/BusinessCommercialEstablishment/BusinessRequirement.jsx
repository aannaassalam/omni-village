...
const BusinessRequirement = ({navigation, route}) => {
  ...
  const {businessEmployee, businessName, businessInvestment, id, name} =
    route.params;
  ...
  const {mutate: add_business, isPending: isAdding} = useMutation({
    mutationFn: addBusiness,
    onSuccess: data => {
      console.log('successsssss save', data), queryClient.invalidateQueries();
      navigation.replace('businessCount');
      setDraftpopup(false), setSavepopup(false);
    },
    onError: error => console.log('error save', error),
  });
  const {mutate: edit_business, isPending: isEditing} = useMutation({
    ...
    <View style={styles.container}>
      ...
      <KeyboardAwareScrollView
        ...
        <SwitchButton
          ...
          secondBtnPress={() => {
            setValues({...values, skill_requirement: false});
          }}
          firstBtnText={t('yes')}
          secondBtntext={t('no')}
        />
        {values?.skill_requirement ? (
          <View style={styles.innerInputView}>
            ...
            <View style={{width: '100%'}}>
              ...
              />
              {errors.type_of_skill && errors.type_of_skill && (
                <Text style={Styles.error2}>{errors.type_of_skill}</Text>
              )}
              <CustomDropdown
                data={business_dropdown?.urgency.map(item => {
                  return {
        ...
        <SwitchButton
          ...
          firstBtnPress={() =>
            setValues({...values, manpower_requirement: true})
          }
          secondBtnPress={() => {
            setValues({...values, manpower_requirement: false});
          }}
          firstBtnText={t('yes')}
          secondBtntext={t('no')}
        />
        {values?.manpower_requirement ? (
          <View style={styles.innerInputView}>
            ...
            <View style={{width: '100%'}}>
              <Input
                label={t(`Number of manpower required`)}
              ...
              />
              {errors.number_of_manpower && errors.number_of_manpower && (
                <Text style={Styles.error2}>{errors.number_of_manpower}</Text>
              )}
              <CustomDropdown
                data={business_dropdown?.urgency.map(item => {
                  return {
              ...
              {touched?.manpower_urgency && errors?.manpower_urgency && (
                <Text style={Styles.error2}>
                  {String(errors?.manpower_urgency)}
                </Text>
              )}
            </View>
          </View>
        ...
        <SwitchButton
          ...
          firstBtnPress={() =>
            setValues({...values, equipment_requirement: true})
          }
          secondBtnPress={() => {
            setValues({...values, equipment_requirement: false});
          }}
          firstBtnText={t('yes')}
          secondBtntext={t('no')}
        />
        {values?.equipment_requirement ? (
          <View style={styles.innerInputView}>
            ...
            <View style={{width: '100%'}}>
              ...
              />
              {errors.equipment_type && errors.equipment_type && (
                <Text style={Styles.error2}>{errors.equipment_type}</Text>
              )}
              <CustomDropdown
                data={business_dropdown?.urgency.map(item => {
                  return {
              ...
              {touched?.equipment_urgency && errors?.equipment_urgency && (
                <Text style={Styles.error2}>
                  {String(errors?.equipment_urgency)}
                </Text>
              )}
            </View>
          </View>
        ...
        {errors.other && errors.other && (
          <Text style={Styles.error2}>{errors.other}</Text>
        )}
        {values?.other !== '' ? (
          <>
            <CustomDropdown
