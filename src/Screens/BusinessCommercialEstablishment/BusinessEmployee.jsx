...
const BusinessEmployee = ({navigation, route}) => {
  ...
  const scheme = yup.object().shape({
    total_employee: yup
      .number()
      .required(t('Total employee is required'))
      .test(
        'coworkers-check',
        t(
        ...
        function (value) {
          const {coworker_inside_village, coworker_outside_village} =
            this.parent;
          return coworker_inside_village + coworker_outside_village <= value;
        },
      ),
    coworker_inside_village: yup
      .number()
      .required(t('Co worker inside village is required')),
    coworker_outside_village: yup
      .number()
      .required(t('Co worker outside village is required')),
  ...
  const {
    ...
    onSubmit: async values => {
      ...
      navigation.navigate('businessInvestment', {
        businessEmployee: new_data,
  ...
  useEffect(() => {
    resetForm({
      values: {
        ...
        coworker_inside_village:
          String(business?.coworker_inside_village || '') || '',
        coworker_outside_village:
          String(business?.coworker_outside_village || '') || '',
  ...
  return (
    <View style={styles.container}>
      ...
      <KeyboardAwareScrollView
        ...
        />
        {errors.total_employee && errors.total_employee && (
          <Text style={Styles.error2}>{errors.total_employee}</Text>
        )}
        <View style={styles.innerInputView}>
          <Divider style={styles.divider2} />
          <View style={{width: '100%'}}>
            <Input
              label={t(`Coworkers within the village neighbourhood`)}
            ...
            <Input
              label={t(`Coworkers from outside`)}
        ...
        <CustomDropdown
          data={
            business_dropdown?.legal_structure
        ...
        {errors.annual_turnover && errors.annual_turnover && (
          <Text style={Styles.error2}>{errors.annual_turnover}</Text>
        )}
        <SwitchButton
          nolabel={false}
          ...
          firstBtnPress={() =>
            setValues({...values, made_profit: true, total_loss: ''})
          }
          secondBtnPress={() => {
            setValues({...values, made_profit: false, total_profit: ''});
          }}
          firstBtnText={t('yes')}
          secondBtntext={t('no')}
        />
            ...
            {values?.made_profit ? (
              <>
                <Input
                  ...
                  isRight={<AcresElement title={user?.currency} />}
                />
                {errors.total_profit && errors.total_profit && (
                  <Text style={Styles.error2}>{errors.total_profit}</Text>
                )}
              </>
            ) : (
              <>
                <Input
                  ...
                  isRight={<AcresElement title={user?.currency} />}
                />
                {errors.total_loss && errors.total_loss && (
                  <Text style={Styles.error2}>{errors.total_loss}</Text>
                )}
              </>
            )}
          </View>
