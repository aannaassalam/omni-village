...
const BusinessInvestment = ({navigation, route}) => {
  ...
  const scheme = yup.object().shape({
    investment_need_so_far: yup
      .string()
      .required('Investment need so far is required'),
    ...
    raw_material_consumption: yup
      .array()
      .of(
        yup.object().shape({
          item: yup.string().required(t('Type is required')),
          quantity: yup.string().required(t('Quantity is required')),
          raw_consumption_unit: yup
            .string()
            .required(t('Raw consumption unit is required')),
        }),
      )
    ...
    fuel_source: yup
      ...
      .of(
        yup.object().shape({
          item: yup.string().required(t('Type is required')),
          quantity: yup.string().required(t('Quantity is required')),
          fuel_source_unit: yup
            .string()
            .required(t('Fuel source unit is required')),
        }),
      )
      .required('Fuel source is required'),
    type_of_infrastructure: yup
      .array()
      .of(yup.string())
      .required('Infrastructure is required'),
    machine_equipment_installed: yup
      .string()
      .required('Machine and equipment installed is required'),
  });
  const {
    handleChange,
    handleSubmit,
    ...
    onSubmit: async values => {
      ...
      if (values.raw_consumption_unit == '') {
        ToastAndroid.show(
          t('Raw consumption unit is required'),
          ToastAndroid.SHORT,
        );
      } else {
        navigation.navigate('businessRequirement', {
          businessInvestment: new_data,
          businessEmployee,
  ...
  useEffect(() => {
    resetForm({
      values: {
        investment_need_so_far:
          String(business?.investment_need_so_far || '') || '',
        water_consumption: String(business?.water_consumption || '') || '',
        energy_consumption: String(business?.energy_consumption || '') || '',
        raw_material_consumption:
          business?.raw_material_consumption?.length > 0
            ? business?.raw_material_consumption.map(item => {
                return {
                  item: item.item,
            ...
            : [],
        fuel_source:
          business?.fuel_source?.length > 0
            ? business?.fuel_source.map(item => {
                return {
                  item: item.item,
                  quantity: String(item.quantity),
        ...
        machine_equipment_installed:
          business?.machine_equipment_installed || '',
      },
    });
    setSelectedStatus(
      business?.raw_material_consumption.map(item => item?.item),
    );
  ...
  return (
    <View style={styles.container}>
      ...
      <KeyboardAwareScrollView
        ...
        <Input
          label={t(`Investment made so far`)}
        ...
        />
        {errors.investment_need_so_far && touched.investment_need_so_far && (
          <Text style={Styles.error2}>{errors.investment_need_so_far}</Text>
        )}
        <Input
        ...
        />
        {errors.water_consumption && errors.water_consumption && (
          <Text style={Styles.error2}>{errors.water_consumption}</Text>
        )}
        <Input
        ...
        />
        {errors.energy_consumption && errors.energy_consumption && (
          <Text style={Styles.error2}>{errors.energy_consumption}</Text>
        )}
        <MultiselectDropdown
          containerStyle={{marginTop: '5%', paddingTop: 0}}
          data={
            business_dropdown?.raw_materials
              ? business_dropdown?.raw_materials.map(item => {
                  return {
              ...
              : [
                  {name: 'Pharmaceutical', key: '6736117ecb51156c2f52383e'},
                  {name: 'IT/Telecom', key: '6736117ecb51156c2f52683e'},
                ]
          }
              ...
              <View style={styles.quantityContainer}>
                {values.raw_material_consumption.map((item, index) => (
                  <>
                    <PurposeInput
                      title={`${
                        business_dropdown?.raw_materials.find(
                          i => item?.item == i?._id,
                        )
                          ? business_dropdown?.raw_materials.find(
                              i => item?.item == i?._id,
                      ...
                      isRight={
                        <CustomDropdown
                          data={
                            business_dropdown?.raw_consumption_unit
                              ? business_dropdown?.raw_consumption_unit.map(
                                  item => {
                                    return {
                                      label:
                              ...
                              : [
                                  ...
                                  {
                                    label: 'kWh',
                                    value: '6736117ecb51156c2f52383b',
                                  },
                                ]
                          }
                    ...
                    {errors.raw_material_consumption &&
                      errors.raw_material_consumption[index]?.quantity && (
        ...
        <MultiselectDropdown
          containerStyle={{marginTop: '5%', paddingTop: 0}}
          data={
            business_dropdown?.fuel_sources
              ? business_dropdown?.fuel_sources.map(item => {
                  return {
                    name: item?.name?.[USER_PREFERRED_LANGUAGE],
                    key: item?._id,
                  };
              ...
              <View style={styles.quantityContainer}>
                {values.fuel_source.map((item, index) => (
                  <>
                    <PurposeInput
                      title={`${
                        business_dropdown?.fuel_sources.find(
                          i => item?.item == i?._id,
                        )
                          ? business_dropdown?.fuel_sources.find(
                              i => item?.item == i?._id,
                        ...
                        <CustomDropdown
                          data={
                            business_dropdown?.fuel_source_unit
                              ? business_dropdown?.fuel_source_unit.map(
                                  item => {
                                    return {
                                      label:
                                        item?.name?.[USER_PREFERRED_LANGUAGE],
                    ...
                    />
                    {errors.fuel_source &&
                      errors.fuel_source[index]?.quantity && (
                        <Text style={Styles.error2}>
                          {errors.fuel_source[index].quantity}
                        </Text>
                      )}
        ...
        <MultiselectDropdown
          ...
          setSelectedd={value => {
            setValues({...values, type_of_infrastructure: value}),
        ...
        {touched?.type_of_infrastructure && errors?.type_of_infrastructure && (
          <Text style={Styles.error2}>
            {String(errors?.type_of_infrastructure)}
          </Text>
        )}
        <Input
          label={t(`Machines & equipments installed`)}
