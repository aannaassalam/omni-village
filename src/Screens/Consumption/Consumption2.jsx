...
import {addConsumptionCorp, fetchConsumptionCorp} from '../../functions/Corps';
import {
  deleteConsumption,
  fetchConsumptions,
...
const Consumption2 = ({navigation, route}) => {
  ...
  const {mutate: deleteConsumptionData, isPending: deleteTreePending} =
    useMutation({
  ...
  const addCrop = () => {
    ...
    if (consumption.find(c => c.consumption_crop_id === dropdownVal._id)) {
      ...
      if (dropdownVal.name.label === 'Others' && otherCrop.length > 0) {
        ...
        setOtherCrop('');
        navigation.navigate('consumptionInput', {
          cropType: consumptionCrop.find(
            i => i?.name === dropdownVal.name?.label,
          ).name,
          typeName: typeName,
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      />
      {/*Top Dashboard  */}
      <CustomDashboard first={t('consumption')} second={typeName} />
