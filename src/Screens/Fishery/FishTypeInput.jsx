...
const FishTypeInput = ({ navigation, route }) => {
    ...
    const schema = yup.object().shape({
        ...
        utilisation_information: yup.object().shape({
            ...
            sold_for_industrial_use: yup.string().required(validation.error.sold_for_industrial_use),
    ...
    const {
        ...
        defaultValues: {
            important_information: {
                number_of_fishes: String(data?.important_information?.number_of_fishes || 0),
                type_of_feed: String(
                    data?.important_information?.type_of_feed || '',
                ),
            },
            utilisation_information: {
                total_feed: String(data?.production_information?.total_feed || 0),
                production_output: String(data?.production_information?.production_output || 0),
                self_consumed: String(data?.production_information?.self_consumed || 0),
                sold_for_industrial_use: String(data?.production_information?.sold_for_industrial_use || 0),
                sold_to_neighbours: String(data?.production_information?.sold_to_neighbours || 0),
    ...
    const onSubmit = () => {
        ...
        let sold_for_industrial_use = parseInt(watch("utilisation_information.sold_for_industrial_use"))
        ...
        if (watch('important_information.type_of_feed') == "" ||
            watch('utilisation_information.expenditure_on_inputs') == "" || watch('utilisation_information.income_from_sale') == "") {
            ...
            if (self_consumed + sold_for_industrial_use + sold_to_neighbours+ wastage + other_value > production_output) {
                        ...
                        .then(
                            () => {
                                Toast.show({
                                    ...
                                    setSavepopup(false),
                                    navigation.navigate('fishery', { totalLand: null, screenName: 'Harvested from Pond' })
                            }
    ...
    useEffect(() => {
        setValue(
            ...
            String(
                ...
                parseInt(getValues('important_information.number_of_fishes'), 10) || '0',
            ),
        );
                    ...
                    {productionInfo ?
                        ...
                        <View style={styles.perContainer}>
                            ...
                            {errors?.utilisation_information?.output.message ? (
                                <Text style={styles.error}>{errors?.utilisation_information?.output.message}</Text>
                            ) : null}
                            <View style={styles.innerInputView}>
                                <Divider style={styles.divider2} />
                                <View style={{ width: '100%' }} >
                                    ...
                                    />
                                    {errors?.utilisation_information?.sold_for_industrial_use?.message ? (
                                        <Text style={styles.error}>
                                            {errors?.utilisation_information?.sold_for_industrial_use?.message}
                                        </Text>
                                    ) : null}
