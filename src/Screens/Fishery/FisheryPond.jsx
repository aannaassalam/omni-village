...
import { addFisheryCorp, addTreeCorp, fetchTreeCorp } from '../../functions/Corps';
    ...
    const addCrop = () => {
        ...
        if (fishery.find(c => c.fishery_crop_id === dropdownVal._id)) {
            ...
            if (dropdownVal.name.label === 'Others' && otherCrop.length > 0) {
                ...
                console.log("i am heleeelkejo")
                setCropModal(false);
                setOtherCrop('');
                navigation.navigate('fishTypeInput', {
                    cropName: fisheryCrops.find(i => i?.name === dropdownVal.name?.label)
                        .name,
                    crop_id: fisheryCrops.find(i => i?.name === dropdownVal.name?.label)._id,
                });
                bottomSheetRef.current.close();
            }
        }
