...
import { addFisheryCorp, addHuntingCorp, addTreeCorp, fetchFisheryCorp, fetchHuntingCorp, fetchTreeCorp } from '../../functions/Corps';
import '../../i18next';
import { useFocusEffect } from '@react-navigation/native';
import { deleteHunting, fetchHuntings } from '../../functions/huntingScreen';

const Hunting2 = ({ navigation, route }) => {
    ...
    const { data: huntingCrops, isLoading } = useQuery({
        queryKey: ['huntingCrop'],
        queryFn: fetchHuntingCorp,
        refetchOnWindowFocus: true,
    });
    const {
        data: hunting,
        isLoading: isHuntingLoading,
        refetch,
        isFetching,
    } = useQuery({
        queryKey: ['huntings'],
        queryFn: fetchHuntings,
        refetchOnWindowFocus: true,
    });
    ...
    const { mutate: deleteHuntingData, isPending: deleteTreePending } = useMutation({
        mutationFn: deleteHunting,
        onSuccess: () => {
    ...
    const addCrop = () => {
        ...
        if (hunting.find(c => c.hunting_crop_id === dropdownVal._id)) {
            setGlobalError('Crop is already added!');
        } else {
            if (dropdownVal.name.label === 'Others' && otherCrop.length > 0) {
                ...
                setOtherCrop('');
                navigation.navigate('huntingType', {
                    cropType: huntingCrops.find(i => i?.name === dropdownVal.name?.label)
                        .name,
                    cropId: huntingCrops.find(i => i?.name === dropdownVal.name?.label)._id,
                });
                bottomSheetRef.current.close();
            }
        }
    ...
    const handleDelete = () => {
        deleteHuntingData(delete_id);
    };

    useEffect(() => {
        setCropType(
            hunting?.map(i => ({
                name: i.hunting_crop.name,
                _id: i.hunting_crop._id,
                data: i,
                status: i.status,
            })),
        );
            ...
            <AddBottomSheet
                ...
                <View style={styles.dropdownSection}>
                    <CustomDropdown4
                        ...
                        data={isLoading ? [] : [...huntingCrops, { _id: 0, name: 'Others' }]}
