...
const OtherEnergy = ({navigation, route}) => {
  ...
  const {mutate: edit_others, isPending: isEditing} = useMutation({
    mutationFn: editOthers,
    onSuccess: data => {
  ...
  const {mutate: add_others, isPending: isAdding} = useMutation({
    mutationFn: addOthers,
    onSuccess: data => {
      queryClient.invalidateQueries();
      console.log('successsssss save', data), navigation.replace('energyFuel');
      setDraftpopup(false), setSavepopup(false);
    ...
    <View style={styles.container}>
      ...
      <KeyboardAwareScrollView
        ...
        <MultiselectDropdown
          ...
          data={
            energy?.source_of_fuels_used?.length > 0
