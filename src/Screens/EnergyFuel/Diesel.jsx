...
const Diesel = ({navigation, route}) => {
  ...
  const scheme = yup.object().shape({
    yearly_petrol_consumption: yup
      .number()
      .required(t('Yearly diesel consumption required')),
    yearly_expenditure_petrol: yup
      .number()
      .required(t('Yearly expenditure on diesel is required')),
  ...
  useEffect(() => {
    resetForm({
      values: {
        yearly_petrol_consumption: String(
          get_type?.yearly_petrol_consumption || '',
        ),
        yearly_expenditure_petrol: String(
          get_type?.yearly_expenditure_petrol || '',
        ),
        purpose_petrol_used_for:
          get_type?.purpose_petrol_used_for.map(item => {
            return {
              type: item.type,
    ...
    setSelectedStatus(
      get_type?.purpose_petrol_used_for.map(item => item.type) || [],
    );
  }, [get_type]);
  ...
  return (
    <View style={styles.container}>
      ...
      <KeyboardAwareScrollView
        ...
        <Input
          label={t(`Yearly consumption of diesel`)}
        ...
        <Input
          label={t(`Yearly expenditure on diesel`)}
        ...
        <MultiselectDropdown
          ...
          data={energy?.purpose_diesel.map(item => {
            return {
              ...
              <View style={styles.quantityContainer}>
                {values.purpose_petrol_used_for.map((item, index) => (
                  <>
                    <PurposeInput
                      title={`${
                        energy?.purpose_diesel.find(i => item?.type == i?._id)
                          ? energy?.purpose_diesel.find(
                              i => item?.type == i?._id,
                    ...
                    {errors.purpose_petrol_used_for &&
                      errors.purpose_petrol_used_for[index]?.quantity && (
                        <Text style={Styles.error2}>
                          {errors.purpose_petrol_used_for[index].quantity}
                        </Text>
                      )}
