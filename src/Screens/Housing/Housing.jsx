...
const Housing = ({navigation}) => {
  ...
  const {mutate: add_housing_by_user, isPending: isAdding} = useMutation({
    mutationFn: addHousingByUser,
    onSuccess: data => {
  ...
  const {mutate: edit_housing_by_user, isPending: isEditing} = useMutation({
    mutationFn: editHousingByUser,
    onSuccess: data => {
      queryClient.invalidateQueries();
  ...
  const scheme = yup.object().shape({
    house_requirements: yup
      .boolean()
      .required(t('House requirements is required')),
  });
  const {
    handleChange,
    handleSubmit,
    ...
    <View style={styles.container}>
      ...
      <View style={styles.mainContainer}>
        <SwitchButton
          ...
          secondBtnPress={() =>
            setValues({...values, house_requirements: false})
          }
          firstBtnText={t('yes')}
          secondBtntext={t('no')}
        />
        {touched?.house_requirements && errors?.house_requirements && (
          <Text style={Styles.error2}>
            {String(errors?.house_requirements)}
          </Text>
        )}
      </View>
