...
const HouseholdRequirement = ({navigation, route}) => {
  ...
  const {house, housingData, housingPhoto, house_id, housing_data} =
    route.params;
  ...
  const handleDraft = () => {
    if (house_id) {
      ...
      add_housing(
        jsonToFormdata({...housingData, ...housingPhoto, ...values, status: 0}),
      );
    }
  };
  ...
  const onSubmit = () => {
    if (house_id) {
      ...
      add_housing(
        jsonToFormdata({...housingData, ...housingPhoto, ...values, status: 1}),
      );
    }
  };
  useEffect(() => {
        ...
        {houseDetails ? (
          <>
            <MultiselectDropdown
              ...
              data={housing_dropdown?.equipment.map(item => {
                return {
            ...
            <MultiselectDropdown
              containerStyle={{marginTop: '5%', paddingTop: 0}}
              data={
                housing_dropdown?.furnishing
                  ? housing_dropdown?.furnishing.map(item => {
                      return {
                        name: item?.name?.[USER_PREFERRED_LANGUAGE],
                        key: item?._id,
                      };
            ...
            {touched?.renovation_requirement &&
              errors?.renovation_requirement && (
                <Text style={Styles.error2}>
                  {String(errors?.renovation_requirement)}
                </Text>
              )}
            {values?.renovation_requirement && (
              <View style={styles.innerInputView}>
                ...
                <View style={{width: '100%'}}>
                  ...
                  />
                  {touched?.renovation_urgency &&
                    errors?.renovation_urgency && (
                      <Text style={Styles.error2}>
                        {String(errors?.renovation_urgency)}
                      </Text>
                    )}
                </View>
              </View>
            ...
            {touched?.expansion_requirement &&
              errors?.expansion_requirement && (
                <Text style={Styles.error2}>
                  {String(errors?.expansion_requirement)}
                </Text>
              )}
              ...
              <View style={styles.innerInputView}>
                ...
                <View style={{width: '100%'}}>
                  ...
                  {touched?.expansion_urgency && errors?.expansion_urgency && (
                    <Text style={Styles.error2}>
                      {String(errors?.expansion_urgency)}
                    </Text>
                  )}
                </View>
              </View>
