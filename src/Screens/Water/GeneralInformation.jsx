...
const GeneralInformation = ({navigation, route}) => {
  ...
  const {mutate: edit_usage, isPending: isEditing} = useMutation({
    mutationFn: editWaterGeneralInfo,
    onSuccess: data => {
  ...
  const {mutate: add_usage, isPending: isAdding} = useMutation({
    mutationFn: addWaterGeneralInfo,
    onSuccess: data => {
      queryClient.invalidateQueries();
      console.log('successsssss save', data), navigation.replace('water');
      setDraftpopup(false), setSavepopup(false);
  ...
  const scheme = yup.object().shape({
    ...
    water_scarcity_severity: yup
      .string()
      .nullable()
      .test(
        'is-required-if-water_scarcity-true',
        t('Severity is required'),
        function (value) {
        ...
        {values?.water_scarcity && (
          <View style={styles.innerInputView}>
            ...
            <View style={{width: '100%'}}>
              <CustomDropdown
                data={water_dropdown?.severity.map(item => {
                  return {
              ...
              />
              {touched?.water_scarcity_severity &&
                errors?.water_scarcity_severity && (
                  <Text style={Styles.error2}>
                    {String(errors?.water_scarcity_severity)}
                  </Text>
                )}
              <MultiselectDropdown
