...
const OtherUsage = ({navigation, route}) => {
  ...
  const scheme = yup.object().shape({
    other_name: yup.string().required(t('Other name is required')),
  ...
  useEffect(() => {
    resetForm({
      values: {
        other_name: get_usage?.other_name || '',
  ...
  return (
    <View style={styles.container}>
      ...
      <KeyboardAwareScrollView
        ...
        <Input
          label={t(`Kindly mention other purpose of water`)}
        ...
        />
        {errors.other_name && errors.other_name && (
          <Text style={Styles.error2}>{errors.other_name}</Text>
        )}
        <CustomDropdown
