...
import {
  addPetrolDieselNatural,
  editPetrolDieselNatural,
  getEnergyByType,
  getEnergyDropdown,
...
import {
  ...
  editOtherPersonal,
  getOtherPersonal,
  getOtherPersonalDropdown,
} from '../../functions/otherPersonalHousehold';
import {fontFamilyRegular} from '../../styles/fontStyle';

const OfficeSupplies = ({navigation, route}) => {
  ...
  const scheme = yup.object().shape({
    personal_care_item_use: yup
      .array()
      .required(t('Office supplies used is required')),
    yearly_expense_for_personal_care: yup
      .number()
      .required(t('Yearly expense for Office supplies is required')),
  ...
  const handleDraft = () => {
    ...
    if (get_other_personal?._id) {
      edit_other_personal({
        ...new_data,
        personal_household_id: get_other_personal._id,
      });
    } else {
  ...
  useEffect(() => {
    resetForm({
      values: {
        personal_care_item_use:
          get_other_personal?.personal_care_item_use || [],
        yearly_expense_for_personal_care:
          String(get_other_personal?.yearly_expense_for_personal_care || '') ||
          '',
        items_produced_locally:
          get_other_personal?.items_produced_locally || false,
        items_produces:
          get_other_personal?.items_produces.map(item => {
            return {
              type: item.type,
    ...
    setSelectedStatus(
      get_other_personal?.items_produces.map(item => item.type) || [],
    );
  }, [get_other_personal]);
  if (isTypeLoading) {
    return (
    ...
    <View style={styles.container}>
      ...
      <KeyboardAwareScrollView
        ...
        <MultiselectDropdown
          ...
          data={
            other_personal?.office_supplies
              ? other_personal?.office_supplies.map(item => {
                  return {
          ...
          infoName={t('What are the Office supplies you use?')}
        />
        {errors.personal_care_item_use && errors.personal_care_item_use && (
          <Text style={Styles.error2}>{errors.personal_care_item_use}</Text>
        )}
        <Input
          label={t(`Total yearly expense for Office supplies`)}
        ...
        <SwitchButton
          nolabel={false}
          label={t(
            'Are there items that you produce locally in village or at home?',
          )}
          selected={values?.items_produced_locally}
          firstBtnPress={() =>
            setValues({...values, items_produced_locally: true})
          }
          secondBtnPress={() => {
            setValues({
              ...values,
        ...
        {values?.items_produced_locally ? (
          <>
            <MultiselectDropdown
              containerStyle={{marginTop: '5%', paddingTop: 0}}
              data={
                other_personal?.office_supplies_produce
                  ? other_personal?.office_supplies_produce.map(item => {
                      return {
                        name: item?.name?.[USER_PREFERRED_LANGUAGE],
                        key: item?._id,
                      };
                    ...
                    {values.items_produces.map((item, index) => (
                      <>
                        <PurposeInput
                          title={`${
                            other_personal?.office_supplies_produce
                              ? other_personal?.office_supplies_produce.find(
                                  i => item?.type == i?._id,
                                )
                                ? other_personal?.office_supplies_produce.find(
                                    i => item?.type == i?._id,
                        ...
                        {errors.items_produces &&
                          errors.items_produces[index]?.quantity && (
                            <Text style={Styles.error2}>
                              {errors.items_produces[index].quantity}
                            </Text>
                          )}
