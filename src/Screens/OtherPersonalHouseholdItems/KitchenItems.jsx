...
const KitchenItems = ({navigation, route}) => {
  ...
  const scheme = yup.object().shape({
    personal_care_item_use: yup
      .array()
      .required(t('Kitchen items used is required')),
    yearly_expense_for_personal_care: yup
      .number()
      .required(t('Yearly expense for Kitchen items is required')),
        ...
        {values?.items_produced_locally ? (
          <>
            <MultiselectDropdown
              containerStyle={{marginTop: '5%', paddingTop: 0}}
              data={
                other_personal?.kitchen_items_produce
                  ? other_personal?.kitchen_items_produce.map(item => {
                      return {
                        name: item?.name?.[USER_PREFERRED_LANGUAGE],
                        key: item?._id,
                      };
                    ...
                    {values.items_produces.map((item, index) => (
                      <>
                        <PurposeInput
                          title={`${
                            other_personal?.kitchen_items_produce
                              ? other_personal?.kitchen_items_produce.find(
                                  i => item?.type == i?._id,
                                )
                                ? other_personal?.kitchen_items_produce.find(
                                    i => item?.type == i?._id,
