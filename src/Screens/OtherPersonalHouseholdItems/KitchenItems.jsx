...
const KitchenItems = ({ navigation, route }) => {
  ...
  const {data: other_personal, isLoading: isDropdownLoading} = useQuery({
    ...
    const scheme = yup.object().shape({
        personal_care_item_use: yup.array().required(t('Kitchen items used is required')),
        yearly_expense_for_personal_care: yup.number().required(t('Yearly expense for Kitchen items is required')),
    ...
    return (
        <View style={styles.container}>
            ...
            <KeyboardAwareScrollView
                ...
                <Input
                    label={t(
                        `Total yearly expense for Kitchen items`
                    )}
                ...
                {values?.items_produced_locally ?
                    <>
                        <MultiselectDropdown
                            containerStyle={{ marginTop: '5%', paddingTop: 0 }}
                            data={other_personal?.kitchen_items_produce
                                ...
                                <View style={{ width: '100%' }}>
                                    <View style={styles.quantityContainer}>
                                        {values.items_produces.map((item, index) => (
                                            <>
                                                <PurposeInput
                                                    title={`${other_personal?.kitchen_items_produce
