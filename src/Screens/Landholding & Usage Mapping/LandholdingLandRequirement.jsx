...
import {
  editLandholding,
  editLandholdingSpecification,
  getLandholdingDropdown,
  getLandholdingSpecification,
...
const LandholdingLandRequirement = ({navigation, route}) => {
  ...
  const {data: landholding_requirement, isLoading: isLandholdingLoading} =
    useQuery({
      queryKey: ['landholding_requirement'],
      queryFn: () => getLandholdingSpecification(),
      refetchOnWindowFocus: true,
    });
  const {mutate: edit_landholding_specification, isPending} = useMutation({
    mutationFn: editLandholdingSpecification,
    onSuccess: data => {
  ...
  const scheme = yup.object().shape({
    ...
    purpose_for_required_land: yup
      .string()
      .required(t('purpose for required land for is required')),
    other_purpose_of_land: yup.string(),
    urgency_required_land: yup
      .string()
      .required(t('urgency for required land is required')),
  });
  const {
    handleChange,
    handleSubmit,
  ...
  useEffect(() => {
    resetForm({
      values: {
        required_area:
          landholding_requirement?.required_area === null
            ? ''
            : String(landholding_requirement?.required_area) || '',
        purpose_for_required_land:
          landholding_requirement?.purpose_for_required_land || '',
        urgency_required_land:
          landholding_requirement?.urgency_required_land || '',
        other_purpose_of_land:
          landholding_requirement?.other_purpose_of_land || '',
      },
    });
  ...
  return (
    <View style={styles.container}>
      ...
      <KeyboardAwareScrollView
        ...
        <Input
          label={t(`Kindly mention the required area`)}
        ...
        />
        {errors.required_area && errors.required_area && (
          <Text style={Styles.error2}>{errors.required_area}</Text>
        )}
        <CustomDropdown
          data={landholding_dropdown?.purpose_requirement.map(item => {
            return {
          ...
          label={t(
            'Kindly mention the purpose for which the land is required?',
          )}
          onChange={value => {
            setValues({
              ...values,
        ...
        />
        {touched?.purpose_for_required_land &&
          errors?.purpose_for_required_land && (
            <Text style={Styles.error2}>
              {String(errors?.purpose_for_required_land)}
            </Text>
          )}
        {landholding_dropdown?.purpose_requirement.find(
          item => item?._id === values?.purpose_for_required_land,
        )?.name?.en === 'other (specify)' && (
          <Input
        ...
        <CustomDropdown
          data={landholding_dropdown?.urgency_requirement.map(item => {
            return {
              label: item?.name?.[USER_PREFERRED_LANGUAGE],
              value: item?._id,
            };
        ...
        {touched?.urgency_required_land && errors?.urgency_required_land && (
          <Text style={Styles.error2}>
            {String(errors?.urgency_required_land)}
          </Text>
        )}
      </KeyboardAwareScrollView>
      <View
