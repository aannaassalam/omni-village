...
const LandSpecification = ({ navigation, route }) => {
    ...
    useEffect(() => {
        ...
        const totalPurpose = land_use ? parseInt(values.total_purpose.length || 0) : []
        const totalStatus = !land_use ? parseInt(values?.status_of_land.length || 0) : [];
                ...
                {values?.land_under_use ?
                            ...
                            <View>
                                {values.purpose_land_utilised_for.map((item, index) => (
                                    <>
                                        <Input
                                            label={t(
                                                `${t('Kindly mention the total land area utilised by')} ${item?.type} `,
                                            )}
                    ...
                    : <>
                        ...
                        {touched?.status_of_land && errors?.status_of_land && (
                            <Text style={Styles.error2}>{String(errors?.status_of_land)}</Text>
                        )}
                            ...
                            <View>
                                {values.purpose_status_of_land.map((item, index) => (
                                    <>
                                        <Input
                                            label={t(
                                                `${t('Kindly mention the total land area not utilised by')} ${item?.type} `,
                                            )}
                                            value={item.total_land_area_utilised}
                                            placeholder={'0'}
                                            fullLength={true}
