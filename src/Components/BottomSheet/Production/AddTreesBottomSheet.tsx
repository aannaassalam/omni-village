...
const AddTreesBottomSheet = ({
  ...
  const snapPoints = React.useMemo(() => ['70%'], []);
   ...
   const {data: trees_crop} = useQuery({
     queryKey: ['trees'],
     queryFn: () =>
       get_crops({country: authState?.country, category: 'tree'}),
   });
          ...
          onPress={async () => {
            if (
              ...
              let data = {
                ...
                category: 'tree', //cultivation, fishery, hunting, poultry, tree
              };
              addCrop(data);
            } else if (
              (crop_name?.name == 'others' && extra_crop_name === null) ||
              ...
              ToastAndroid.show('Please enter trees name', ToastAndroid.SHORT);
            } else {
              setModalVisible(!modalVisible), bottomsheetRef.current.close();
              await setData({
...
export default AddTreesBottomSheet;
